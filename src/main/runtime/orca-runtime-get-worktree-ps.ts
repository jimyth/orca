...
import { zcodeStructuredPermissionModeForSettings } from '../zcode/zcode-structured-permission-policy'
...
export class OrcaRuntimeWithGetWorktreePs extends OrcaRuntimeWithStructuredAgentSessionRecoverTuiOwner {
  ...
  async ensureStructuredAgentSessionHost(): Promise<void> {
    await installStructuredAgentSessionHost({
      ...
      resolveZcodeLaunchEnv: () =>
        resolveTuiAgentLaunchEnv('zcode', this.requireStore().getSettings().agentDefaultEnv),
      ...
      resolveZcodePermissionMode: () =>
        zcodeStructuredPermissionModeForSettings(this.requireStore().getSettings()),
